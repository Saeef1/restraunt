...
const Cart = () => {
  ...
  if (cartItems.length === 0) {
    return (
      <div className="h-[calc(100vh-6rem)] flex flex-col items-center justify-center text-white">
        <div className="text-center">
          ...
          <button 
            onClick={() => router.push('/')}
            className="bg-[crimson] hover:bg-red-700 text-white px-6 py-3 rounded-lg font-bebas text-lg transition-colors"
          >
            Continue Shopping
          </button>
        </div>
  ...
  return (
    <>
      <div className="h-[calc(100vh-6rem)] mt-32 lg:flex-row md:h-[calc(100vh-6rem)] flex flex-col items-center text-white">
        <div className="h-1/2 p-4  flex flex-col justify-center overflow-y-scroll md:h-full md:mt-7 lg:w-2/3 lg:px-20 xl:px-10">
          {cartItems.map((item) => (
            <div 
              key={item.id}
              className="flex items-center justify-between bg-gray-800/50 backdrop-blur-sm border border-red-900/30 rounded-lg p-4 mb-4"
            >
        ...
        <div className="h-1/2 p-4 flex flex-col gap-4 mt-5 justify-center lg:h-full lg:w-1/3 lg:px-20 xl:px-20">
          ...
          <div className="flex justify-between bg-gray-800/50 backdrop-blur-sm border border-red-900/30 rounded-lg p-4">
            ...
            <span className="text-xl font-bold text-[gold]">Rs {total}/-</span>
          </div>
          <button 
            onClick={handleCheckout}
            disabled={cartItems.length === 0}
            className="bg-[crimson] hover:bg-red-700 disabled:bg-gray-600 text-white px-8 py-4 rounded-md font-bebas text-lg transition-colors self-end"
          >
            CHECKOUT
          </button>
        </div>
